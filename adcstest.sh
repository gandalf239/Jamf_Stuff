...
# 
#     Re-D"o the install once you have things tested so you
...
ADCS_CA_template=""   # On an enterprise CA
# ADCS_CA_template=""       # On a standalone CA, there is no template

# Folder where you put the server and client certs from the ADCS Connector install. 
# (relative to this script's path). 
authenticationCertSubFolder='certs'
serverCertFileName='adcs-proxy-ca.cer'
...
# E.g., ""Your_Computer_Name""
# Define the array of hostnames for which you want to request certificates
hostnames=("")
...
for hostname in "${hostnames[@]}"; do
  ...
  # Replace the subject with the current hostname in the loop
  subject="$hostname"

  # (The rest of your script follows, and it will now request a cert for each hostname in the array)
  
  # Use the variables to generate the CSR or handle the cert request for each computer
  
  # For example, here you might use `openssl` or whatever tool is part of your process
  # The following is a mock command to request a certificate:
  # (This is where you'd put your actual cert request logic)
  
  # openssl req -new -key client-key.key -out "$subject.csr" -subj "/CN=$subject"
  
  # Simulate certificate request (replace this with your actual cert request command)
  echo "Requesting cert from CA $ADCS_CA_hostname using template $ADCS_CA_template for $subject"
  
  # Simulate a certificate file creation (replace this with actual process)
  touch "$hostname-cert.p12"
  echo "Certificate saved as $hostname-cert.p12"
  
done


# subject=""ITS-469218-L".one.phoenix.gov"
...
if [[ -d "$iisCertsDir" ]]; then   # Use lowercase 'd' and correct the placement of the double quotes
...
# Handle response
if [ $? -eq 0 ]; then
  echo "[ok] Request sent successfully."
else
...
# If you wanted to feed json in from the file, you could use @ in your curl --D"ata 
#--D"ata "@${jsonPayloadFilePath}" \
...
#  https://blogs.technet.microsoft.com/nexthop/2012/12/17/updated-creating-a-certificate-revocation-L"ist-D"istribution-point-for-your-internal-certification-authority/
